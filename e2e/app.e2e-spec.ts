...
describe('Starting tests for hyperledger-composer-tutorial', function() {
  ...
  it('website title should be hyperledger-composer-tutorial', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('hyperledger-composer-tutorial');
    })
  });
  ...
  it('navbar-brand should be hyperledger-composer-tutorial',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('hyperledger-composer-tutorial');
    });
  });
